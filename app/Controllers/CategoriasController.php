...
if (!isset($pdo)) {
    throw new RuntimeException('O objeto $pdo não está disponível no CategoriasController.');
}
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if (!tokenCsrfCategoriasValido($_POST['csrf_token'] ?? null)) {
        ...
        } elseif (categoriaDuplicada($pdo, $carteiraId, $dados['nome'], $dados['tipo'])) {
        ...
        $categoriaAtual = buscarCategoriaCarteira($pdo, $carteiraId, $id);

        if ($id <= 0 || !$categoriaAtual) {
            $resultado = ['sucesso' => false, 'mensagem' => 'Categoria não encontrada.'];
        } elseif (!$dados['valida']) {
            $resultado = ['sucesso' => false, 'mensagem' => $dados['mensagem']];
        } elseif (categoriaDuplicada($pdo, $carteiraId, $dados['nome'], $dados['tipo'], $id)) {
            $resultado = ['sucesso' => false, 'mensagem' => 'Já existe outra categoria com esse nome e tipo.'];
        } else {
            $totalVinculos = contarTransacoesCategoria($pdo, $carteiraId, $id);
        ...
        try {
            if ($id <= 0 || !excluirCategoria($pdo, $carteiraId, $id)) {
                $resultado = ['sucesso' => false, 'mensagem' => 'Categoria não encontrada.'];
            } else {
...
$categoriasList = listarCategoriasComTotais($pdo, $carteiraId);
...
require dirname(__DIR__) . '/Views/categorias/index.php';
