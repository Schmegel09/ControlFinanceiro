...
/** @var string $edicaoAction */
/** @var string $csrfToken */
/** @var string $transacoesJson */
/** @var string $categoriasJson */
    ...
    <div class="tx-modal-content">
        ...
        <form method="post" action="<?= htmlspecialchars($edicaoAction, ENT_QUOTES, 'UTF-8') ?>" id="tx-edit-form">
            ...
            <div class="tx-form-grid">
                ...
                <div class="tx-form-group">
                    <label for="tx-edit-data">Data da primeira parcela</label>
                ...
                <div class="tx-form-group">
                    <label for="tx-edit-valor">Valor total (R$)</label>
                    <input type="text" id="tx-edit-valor" name="valor" inputmode="decimal" required>
                </div>

                <div class="tx-form-group" id="tx-edit-installments-group">
                    <label for="tx-edit-parcelas">Quantidade de parcelas</label>
                    <input
                        type="number"
                        id="tx-edit-parcelas"
                        name="parcelas"
...
<script>
    ...
    const txParcelas = document.getElementById('tx-edit-parcelas');
    const txParcelasGrupo = document.getElementById('tx-edit-installments-group');
    const txAvisoParcela = document.getElementById('tx-installment-note');
    ...
    function atualizarParcelamentoEdicao() {
        const permiteParcelas = txTipo.value === 'despesa';
        txParcelasGrupo.hidden = !permiteParcelas;
        txParcelas.disabled = !permiteParcelas;

        if (!permiteParcelas) {
            txParcelas.value = '1';
        }

        const totalParcelas = Math.max(1, Number.parseInt(txParcelas.value, 10) || 1);
        if (permiteParcelas && totalParcelas > 1) {
            txAvisoParcela.textContent = `Ao salvar, todas as ${totalParcelas} parcelas serão recalculadas a partir do valor total e da primeira data.`;
            txAvisoParcela.classList.add('visible');
        } else {
            txAvisoParcela.textContent = 'Ao salvar, todo o lançamento será atualizado.';
            txAvisoParcela.classList.add('visible');
        }
    }
    ...
    function abrirEdicao(id, botao = null) {
        ...
        document.getElementById('tx-edit-data').value = transacao.data_inicio || transacao.data;
        document.getElementById('tx-edit-valor').value = String(transacao.valor_original || transacao.valor).replace('.', ',');
        document.getElementById('tx-edit-descricao').value = transacao.descricao || '';
        txParcelas.value = String(Math.max(1, Number(transacao.total_parcelas || 1)));
        preencherCategoriasEdicao(txCategoriaSelecionada, txCategoriaLegada);
        atualizarParcelamentoEdicao();
    ...
    txParcelas.addEventListener('input', atualizarParcelamentoEdicao);
